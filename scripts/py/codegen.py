    ...
    for struct_name in tr._registed_struct_types:
        ...
        for mem_name in struct_type._member:
            ...
            for key in mems_dict:
                ...
                _add_line(
                    f"def {mem_name}(self{print_py_args_decl(func._args, False)}):"
                )
                _line_indent += 1
                if func._ret_type:
                    func_call = "return "
...
"""
    ...
    for struct_name in tr._registed_struct_types:
        ...
        _line_indent += 1
        _add_line(
            "ModuleRegister::module_addref(env ? env : luisa::current_executable_path(),*dynamic_module);"
        )
    ...
    _line_indent -= 1
    _add_line("}")
    _add_line(f"static ModuleRegister _{export_func_name}({export_func_name});")
    return _result
