...
void OfflinePTPass::update(Pipeline const& pipeline, PipelineContext const& ctx)
{
    ...
    const auto& jitter_data = ctx.mut.states.read<JitterData>();
    const auto& cam_data = ctx.mut.states.read<CameraData>();
    ...
    {
        (*ctx.cmdlist) << (*clear_hashgrid)(key_buffer, value_buffer, 0).dispatch(key_buffer.size());
    }
    for (auto i : vstd::range(ptSettings.offline_spp))
    {
        ...
        (*ctx.cmdlist) << (*clear_ptr_buffer)(multibounce_buffer_counter.view(), 0).dispatch(1);
        ...
        (*ctx.cmdlist) << offline_multibounce::dispatch_shader(
        ...
        (*ctx.cmdlist) << (*integrate_hashgrid)(
#ifdef OUTPUT_DENOISE_TRAINING_DATA
                              pass_ctx->denoise_buffer,
#endif
                              geo_buffer,
                              ...
                              .dispatch(frameSettings.render_resolution);
        (*ctx.cmdlist) << (*clear_hashgrid)(key_buffer, value_buffer, max_accum).dispatch(key_buffer.size());
    }
    // TODO: spp collection
#ifdef OUTPUT_DENOISE_TRAINING_DATA
    pass_ctx->train_data.resize_uninitialized(pass_ctx->denoise_buffer.size());
    (*ctx.cmdlist) << pass_ctx->denoise_buffer.copy_to(pass_ctx->train_data.data());
#endif
}
