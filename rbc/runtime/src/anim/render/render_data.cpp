...
namespace rbc {
void SkinBufferResource::Create(RG::RenderGraph &Graph, CGPUBufferDescriptor Desc) {
    if (cgpu_buffer_resource_ != nullptr) {
        cgpu_wait_queue_idle(Graph.get_gfx_queue());
        for (auto q : Graph.get_cmpt_queues()) {
            cgpu_wait_queue_idle(q);
        }
        cgpu_free_buffer(cgpu_buffer_resource_);
        cgpu_buffer_resource_ = nullptr;
    }
    RG::RenderGraphBackend &graph_backend = static_cast<RG::RenderGraphBackend &>(Graph);
    auto cgpu_device = graph_backend.get_backend_device();
    cgpu_buffer_resource_ = cgpu_create_buffer(cgpu_device, &Desc);
    tracker_.reset();
}

void SkinBufferResource::GetRDGResource(RG::RenderGraph &Graph, const char8_t *name, uint64_t Count, uint64_t Stride) {
    if (frame_index_ != Graph.get_frame_index()) {
        buffer_handle_ = Graph.create_buffer(
            [=, this](RG::RenderGraph &g, RG::BufferBuilder &builder) {
                builder.set_name(name)
                    .import(cgpu_buffer_resource_, tracker_.get_last_state())
                    .allow_shader_readwrite()
                    .structured(0, Count, Stride);
            });
        frame_index_ = Graph.get_frame_index();
        Graph.track_state(buffer_handle_, tracker_);
    }
}

void SkinBufferResource::Swap(SkinBufferResource &rhs) {
    const auto rhs_buf = rhs.cgpu_buffer_resource_;
    const auto rhs_tracker_state = rhs.tracker_.get_last_state();
    const auto rhs_frame_index = rhs.frame_index_;

    rhs.cgpu_buffer_resource_ = cgpu_buffer_resource_;
    rhs.tracker_.reset(tracker_.get_last_state());
    rhs.frame_index_ = frame_index_;

    cgpu_buffer_resource_ = rhs_buf;
    tracker_.reset(rhs_tracker_state);
    frame_index_ = rhs_frame_index;
}

void SkinBufferResource::SafeRelease(RG::RenderGraph &Graph) {
    if (cgpu_buffer_resource_ != nullptr) {
        cgpu_wait_queue_idle(Graph.get_gfx_queue());
        for (auto q : Graph.get_cmpt_queues())
            cgpu_wait_queue_idle(q);
...
void SkinBufferResource::Release() {
    if (cgpu_buffer_resource_) {
        cgpu_free_buffer(cgpu_buffer_resource_);
        cgpu_buffer_resource_ = nullptr;
    }
}

void SkeletalMeshRenderData::InitializeWithRefSkeleton(const skr::ReferenceSkeleton &InRefSkeleton) {
    // 静态，应该跟随skin初始化
    required_bones.resize_default(InRefSkeleton.NumJoints());
    for (auto i = 0; i < InRefSkeleton.NumJoints(); ++i) {
        required_bones[i] = i;
    }
}

void SkelMeshRenderDataLOD::Initialize(const MeshResource *InMeshResource) {
    // 因为只有vertex buffer和动态相关，所以只有一个buffer
    buffers.resize_zeroed(1);       // CPU Buffers
...
void SkelMeshRenderDataLOD::InitResources(SkeletalMeshRenderData *InRenderData, CGPUDeviceId InDevice) {
    // 动态，应该跟随RenderObject初始化
    const auto *InMeshResource = InRenderData->static_mesh_;
    render_data_ = InRenderData;
    SKR_ASSERT(InMeshResource);
