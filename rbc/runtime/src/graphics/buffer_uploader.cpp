...
void BufferUploader::load_shader(luisa::fiber::counter& counter)
{
    ...
    luisa::fiber::schedule([this, counter]() {
        ShaderManager::instance()->load("texture_process/uploader_align4cp2.bin", _align4_copy);
        counter.done();
    });

    luisa::fiber::schedule([this, counter]() {
        ShaderManager::instance()->load("texture_process/uploader_align8cp2.bin", _align8_copy);
        counter.done();
    });

    luisa::fiber::schedule([this, counter]() {
        ShaderManager::instance()->load("texture_process/uploader_align16cp2.bin", _align16_copy);
        counter.done();
    });
}
...
)
{
...
#ifndef NDEBUG
        LUISA_ASSERT(value.datas.size_bytes() % sizeof(T) == 0, "Bad size");
#endif
        auto data_buffer = temp_buffer.allocate_upload_buffer<T>(value.datas.size_bytes() / sizeof(T), 16);
    ...
    break;
    default:
        LUISA_ERROR("Invalid alignment {}, require 4, 8 or 16.", key.struct_align);
        break;
    }
}
