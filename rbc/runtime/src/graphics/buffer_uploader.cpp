...
void BufferUploader::commit_cmd(
    ...
    for (auto &i : value.indices_map_bytes) {
        value.indices_vec_bytes.emplace_back(i);
    }
    value.indices_map_bytes.clear();
    auto ind_buffer = temp_buffer.allocate_upload_buffer<uint>(value.indices_vec_bytes.size(), 16);
    memcpy(ind_buffer.mapped_ptr(), value.indices_vec_bytes.data(), value.indices_vec_bytes.size_bytes());
    ...
    switch (key.struct_align) {
        case 2: {
            cmdlist << (*_align2_copy)(
                           ...
                           ind_buffer.view)
                           .dispatch(value.indices_vec_bytes.size(), key.struct_size / 2u);
        } break;
        case 4: {
            cmdlist << (*_align4_copy)(
                           ...
                           ind_buffer.view)
                           .dispatch(value.indices_vec_bytes.size(), key.struct_size / 4u);
        } break;
        case 8: {
            cmdlist << (*_align8_copy)(
                           ...
                           ind_buffer.view)
                           .dispatch(value.indices_vec_bytes.size(), key.struct_size / 8u);
        } break;
        case 16: {
            cmdlist << (*_align16_copy)(
                           ...
                           ind_buffer.view)
                           .dispatch(value.indices_vec_bytes.size(), key.struct_size / 16u);
        } break;
        default:
