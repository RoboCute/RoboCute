...
public:
    TexStreamCallback(Load&& load, UnLoad&& unload, Check&& check)
        : _load(std::forward<Load>(load))
        , _unload(std::forward<UnLoad>(unload))
        , _check(std::forward<Check>(check))
    {
    }
...
TexStreamManager::TexStreamManager(
    ...
    , _bdls_mng(bdls_mng)
    , _level_buffer(device, 65536 * sizeof(uint))
    , _allocate_size_limits(memory_limit)
    , _memoryless_threshold(memoryless_threshold)
...
) -> LoadResult
{
    ...
    auto bindless_idx = _bdls_mng.enqueue_sparse_image(img, sampler);
...
)
{
    ...
    auto& v = iter.value();
    _bdls_mng.dequeue_sparse_image(v->img);
...
TexStreamManager::SparseHeap::SparseHeap(
    ...
    : self(self)
    , heap(self->_device.allocate_sparse_texture_heap(size))
    , size(size)
{
