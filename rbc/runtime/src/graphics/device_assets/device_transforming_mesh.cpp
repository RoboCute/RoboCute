...
void DeviceTransformingMesh::async_load(RC<DeviceMesh> origin_mesh, bool init_buffer)
{
    _origin_mesh = std::move(origin_mesh);
    auto inst = AssetsManager::instance();
    _gpu_load_frame = std::numeric_limits<uint64_t>::max();
    inst->load_thd_queue.push([init_buffer, this_shared = RC{ this }](LoadTaskArgs const& args) {
        auto inst = AssetsManager::instance();
        ...
        if (init_buffer)
        {
            ...
            args.cmdlist
                ...
                << ptr->_last_vertex.view().copy_from(pos_buffer);
        }
    });
}
} // namespace rbc