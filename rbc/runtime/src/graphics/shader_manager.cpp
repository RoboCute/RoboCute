...
void ShaderManager::create_instance(Device& device, luisa::filesystem::path const& shader_path, bool use_lmdb)
{
    if (_shader_manager_instance.has_value())
    {
    ...
    if (use_lmdb)
        _shader_manager_instance->set_lmdb_mode();
}
...
luisa::string_view ShaderManager::_path_to_key(luisa::filesystem::path const& path, luisa::string& can_path_str, luisa::string& buffer) const
{
    ...
    if (_use_lmdb)
    {
        can_path_str = luisa::to_string(path);
        key_strview = can_path_str;
#ifdef _WIN32
        for (auto& i : can_path_str)
        {
            if (i == '\\')
            {
                i = '/';
...
struct ReloadLogic {
    ...
    ReloadLogic(ReloadLogic&& rhs)
        ...
        , parent_path(std::move(rhs.parent_path))
    {
    }
...
)
{
    ...
    return fiber::async_parallel(_shaders.size(), ReloadLogic{ device, _shaders.begin(), _use_lmdb ? (std::filesystem::path const*)nullptr : &_shader_path });
}
