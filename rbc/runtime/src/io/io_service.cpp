...
static constexpr size_t staging_size = 64ull * 1024ull * 1024ull;
...
)
{
    ...
    auto new_ser = new (vengine_malloc(sizeof(IOService))) IOService{ device, src_type };
...
)
{
    ...
    {
    case QueueType::DX12:
        DStorageStream::init_dx12(runtime_dir, staging_size, force_hdd);
        break;
    default:
        DStorageStream::init_fallback(staging_size);
        break;
    }
...
IOService::IOService(Device& device, DStorageSrcType src_type)
...
void IOService::_execute_cmdlist(IOCommandList& cmd, uint64_t timeline)
{
    ...
    split_commands(cmds, extra_cmds);
    cmds.size();
...
void IOService::split_commands(vector<IOCommand>& commands, vector<IOCommand>& extra_commands)
{
...
uint64_t IOService::execute(IOCommandList&& cmdlist)
{
    uint64_t timeline;
    {
        std::lock_guard lck{ _cmd_mtx };
        ...
        _execute_cmdlist(cmdlist, timeline);
    }
    return timeline;
}
} // namespace rbc