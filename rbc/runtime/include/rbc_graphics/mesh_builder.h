...
struct MeshBuilderBase {
    ...
    bool contained_normal() const;
    bool contained_tangent() const;
    uint uv_count() const;
    uint submesh_count() const;
    uint indices_count() const;
    luisa::span<uint const> get_submesh_indices(size_t submesh_index) const;
    luisa::string check() const; // return: error message
    // property write
