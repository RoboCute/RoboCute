...
struct RBC_RUNTIME_API RenderDevice {
    ...
    // init & shutdown
    void init(
        luisa::compute::Context &&ctx, luisa::string_view backend = "dx", bool headless = false, bool require_async_stream = true, bool require_io_service = true, bool gpu_dump = false, void *external_device = nullptr, size_t device_index = std::numeric_limits<size_t>::max());
    template<typename T>
