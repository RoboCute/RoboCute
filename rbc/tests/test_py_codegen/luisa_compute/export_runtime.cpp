...
void export_runtime(py::module &m) {
    ...
    py::class_<ContextRef>(m, "Context")
        .def(py::init<>())
        .def("create_device", [](ContextRef &self, luisa::string_view backend_name) {
            ...
            return ManagedDevice(Device{rb->lc_device()});
        })
        .def("set_shader_path", [](ContextRef &self, std::string const &str) {
