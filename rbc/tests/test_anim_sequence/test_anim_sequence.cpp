...
int disabled_main(int argc, char *argv[]) {
    ...
    // Create animation sequence resource and set meta fields (dependencies)
    auto anim = rbc::world::create_object<AnimSequenceResource>();
    anim->ref_skel = skel;// Inject skeleton dependency
    // anim->anim_name = "ArmatureAction";  // Optional: specify animation name
    // anim->sampling_rate = 30.0f;  // Optional: specify sampling rate
    ...
    // Note: anim holds a ref to skel via anim->ref_skel, so we must delete anim first
    // to avoid double-free. When anim is deleted, ref_skel's destructor will release
    // the reference to skel.
    std::cout << std::endl
              << ">>> Cleaning up..." << std::endl;
    
    std::cout << "Animation RC count before delete: " << anim->rbc_rc_count() << std::endl;
    std::cout << "Skeleton RC count (held by anim): " << skel->rbc_rc_count() << std::endl;
    
    // Delete anim first - this will also release the ref to skel
    anim->rbc_rc_delete();
    std::cout << "Animation deleted" << std::endl;
    
    // skel is already deleted when anim->ref_skel was destroyed
    // skel->rbc_rc_delete();  // DO NOT call this - would be double-free
    std::cout << "Skeleton deleted (via anim->ref_skel release)" << std::endl;
