...
suite<"Basic|Hash Map"> BasicHashMapTestSuite = [] {
// ============================================
...
"hashmap_try_emplace_multiple"_test = [] {
    ...
    // Verify values
    expect(static_cast<bool>(map.find(1).value() == 1.1_d));
    expect(static_cast<bool>(map.find(2).value() == 2.2_d));
    expect(static_cast<bool>(map.find(3).value() == 3.3_d));
};

"hash.1_dmap_force_emplace"_test = [] {
...
"hashmap_indexer_key_value_access"_test = [] {
    ...
    expect(static_cast<bool>(iter.value() == 3.14159_d));
    
    // Modify value through indexer
    iter.value() = 2.71828;
    expect(static_cast<bool>(map.find("pi").value() == 2.71828_d));
};
