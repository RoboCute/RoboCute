...
private:
    ...
    uint64_t _md5[2];
    Type(luisa::string_view name, uint64_t const md5[2])
        : _name(name) {
        _md5[0] = md5[0];
        _md5[1] = md5[1];
    }
public:
    ...
    static Type get() {
        return Type{T::_zz_typename_, T::_zz_md5_};
    }
...
namespace luisa {
template<>
struct hash<rbc::Type> {
    using is_avalanching = void;
    [[nodiscard]] constexpr uint64_t operator()(const rbc::Type &value) const noexcept {
        return value.hash();
    }
};
}// namespace luisa
...
template<>
struct compare<rbc::Type> {
    int32_t operator()(rbc::Type const &a, rbc::Type const &b) const {
        if (a == b) return 0;
        if (a < b) return -1;
        return 1;
    }
