...
static bool parse_mesh(tinyxml2::XMLElement *mesh_elem, MeshBuilder &mesh_builder) {
    ...
    // Check and resize normals to match position size (only if not empty)
    ...
    // Check and resize tangents to match position size (only if not empty)
    ...
    // Check and resize UVs to match position size (only if not empty)
    for (size_t i = 0; i < mesh_builder.uvs.size(); ++i) {
        if (!mesh_builder.uvs[i].empty() && mesh_builder.uvs[i].size() != position_size) {
