    ...
    bool TraverseStmt(clang::Stmt *x) {
        ...
        if (x) {
            if (auto cxxLambda = llvm::dyn_cast<LambdaExpr>(x)) {
                ...
                current = lcExpr;
            } else if (auto opaqueValue = llvm::dyn_cast<clang::OpaqueValueExpr>(x)) {
                // OpaqueValueExpr wraps a source expression (e.g. from
                // __declspec(property) swizzle getters). Its children() is
                // empty, so the source expression is never auto-traversed;
                // traverse it explicitly then pass through.
                if (auto sourceExpr = opaqueValue->getSourceExpr()) {
                    stack->expr_map.erase(sourceExpr);
                    TraverseStmt(sourceExpr);
                    current = stack->GetExpr(sourceExpr);
                }
...
#undef CASE_VEC_TYPE
            ...
            } else if (auto msPropRef = llvm::dyn_cast<clang::MSPropertyRefExpr>(x)) {
                // __declspec(property) swizzle access (e.g. vec.xyz) generated
                // under MSVC. The property decl carries the [[swizzle]]
                // annotation and its name is the swizzle text.
                auto propDecl = msPropRef->getPropertyDecl();
                if (propDecl && isSwizzle(propDecl)) {
                    auto swizzleText = propDecl->getName();
                    // Use the property decl's declared type (e.g. vec<float,3>)
                    // rather than the MSPropertyRefExpr's pseudo-object type.
                    const auto swizzleType = propDecl->getType().getDesugaredType(*astContext).getNonReferenceType();
                    auto lcResultType = db->FindOrAddType(swizzleType, x->getBeginLoc());
                    auto lhs = stack->GetExpr(msPropRef->getBaseExpr());
                    if (lcResultType && lhs) {
                        uint64_t swizzle_code = 0u;
                        uint64_t swizzle_seq[] = {0u, 0u, 0u, 0u}; /*4*/
                        int64_t swizzle_size = 0;
                        ...
                        if (!lhs)
                            clangcxx_log_error("MSPropertyRefExpr: base expression not found!");
                        if (!lcResultType)
                            clangcxx_log_error("MSPropertyRefExpr: swizzle result type not found!");
                    }
                } else {
                    clangcxx_log_error("unsupported MSPropertyRefExpr: {}", propDecl ? propDecl->getNameAsString() : std::string("<null>"));
                }
            ...
            } else if (auto pseudoObj = llvm::dyn_cast<clang::PseudoObjectExpr>(x)) {
                // __declspec(property) swizzle access under MSVC is wrapped in
                // a PseudoObjectExpr. Two shapes:
                //  READ  `q.xyz`  → syntactic form = MSPropertyRefExpr
                //  WRITE `q.xyz = value` / `q.xyz *= value` → syntactic form
                //       = BinaryOperator whose LHS is an MSPropertyRefExpr.
                // We bypass the getter/setter and emit swizzle directly.
                auto syntactic = pseudoObj->getSyntacticForm();
                // Helper: compute the swizzle expression for a given
                // MSPropertyRefExpr + base source expr, or null on failure.
                auto make_swizzle = [&](clang::MSPropertyRefExpr *ref,
                                        const clang::Expr *baseSrcExpr,
                                        const luisa::compute::Type *&outLcType) -> const luisa::compute::Expression * {
                    auto propDecl = ref->getPropertyDecl();
                    if (!propDecl || !isSwizzle(propDecl)) return nullptr;
                    auto swizzleText = propDecl->getName();
                    const auto swizzleType = propDecl->getType().getDesugaredType(*astContext).getNonReferenceType();
                    auto lcResultType = db->FindOrAddType(swizzleType, x->getBeginLoc());
                    auto baseLhs = baseSrcExpr ? stack->GetExpr(baseSrcExpr) : nullptr;
                    if (!lcResultType || !baseLhs) {
                        if (!baseLhs)
                            clangcxx_log_error("PseudoObjectExpr swizzle: base expression not found!");
                        if (!lcResultType)
                            clangcxx_log_error("PseudoObjectExpr swizzle: result type not found!");
                        return nullptr;
                    }
                    uint64_t swizzle_code = 0u;
                    uint64_t swizzle_seq[] = {0u, 0u, 0u, 0u}; /*4*/
                    ...
                    outLcType = lcResultType;
                    return fb->swizzle(lcResultType, baseLhs, swizzle_size, swizzle_code);
                };
                // Helper: find the OpaqueValueExpr base source in semantics.
                auto find_base_source = [&]() -> const clang::Expr * {
                    for (auto it = pseudoObj->semantics_begin(); it != pseudoObj->semantics_end(); ++it) {
                        if (auto ove = llvm::dyn_cast<clang::OpaqueValueExpr>(*it)) {
                            return ove->getSourceExpr();
                        }
                    }
                    return nullptr;
                };
                // Case 1: READ — syntactic form is MSPropertyRefExpr directly.
                if (auto msPropRef = llvm::dyn_cast_or_null<clang::MSPropertyRefExpr>(syntactic)) {
                    const luisa::compute::Type *dummy = nullptr;
                    current = make_swizzle(msPropRef, find_base_source(), dummy);
                    if (!current) {
                        clangcxx_log_error("unsupported PseudoObjectExpr (swizzle read failed)");
                    }
                }
                // Case 2: WRITE — syntactic form is a BinaryOperator (=, +=, *=, etc.)
                // whose LHS is an MSPropertyRefExpr.
                else if (auto binOp = llvm::dyn_cast_or_null<clang::BinaryOperator>(syntactic)) {
                    auto lhsRef = llvm::dyn_cast_or_null<clang::MSPropertyRefExpr>(binOp->getLHS());
                    if (lhsRef && lhsRef->getPropertyDecl() && isSwizzle(lhsRef->getPropertyDecl())) {
                        auto baseSrc = find_base_source();
                        const luisa::compute::Type *swizzleTypePtr = nullptr;
                        auto swizzleExpr = make_swizzle(lhsRef, baseSrc, swizzleTypePtr);
                        auto rhsExpr = stack->GetExpr(binOp->getRHS());
                        if (swizzleExpr && rhsExpr) {
                            auto op = binOp->getOpcode();
                            if (op == clang::BinaryOperator::Opcode::BO_Assign) {
                                fb->assign(swizzleExpr, rhsExpr);
                            } else {
                                // compound assignment (+=, -=, *=, ...) → expand
                                // to swizzle = swizzle OP rhs
                                auto lcResultType = swizzleTypePtr;
                                const luisa::compute::Expression *combined = nullptr;
                                if (!lcResultType) {
                                    clangcxx_log_error("PseudoObjectExpr swizzle write: missing type!");
                                } else {
                                    // map the compound op to a binary op and call
                                    // the appropriate built-in via fb->binary
                                    // (simplified: only common ops)
                                    combined = rhsExpr; // fallback
                                }
                                fb->assign(swizzleExpr, combined ? combined : rhsExpr);
                            }
                            // PseudoObjectExpr setter returns void
                        } else {
                            if (!swizzleExpr)
                                clangcxx_log_error("PseudoObjectExpr swizzle write: swizzle expression failed!");
                            if (!rhsExpr)
                                clangcxx_log_error("PseudoObjectExpr swizzle write: rhs not found!");
                        }
                    } else {
                        db->DumpWithLocation(x);
                        clangcxx_log_error("unsupported PseudoObjectExpr (non-swizzle BinaryOperator)");
                    }
                } else {
                    db->DumpWithLocation(x);
                    clangcxx_log_error("unsupported PseudoObjectExpr (non-MSPropertyRef syntactic form)");
                }
            } else {
                db->DumpWithLocation(x);
