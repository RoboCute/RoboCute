...
#else
	col *= args.hdr_display_multiplier;
	col = pow(col, args.gamma);
	if (args.use_hdr10) {
		// float3x3 adobe_rgb_to_rec2020 = transpose(float3x3(0.877334, 0.077494, 0.045172, 0.096623, 0.891527, 0.011850, 0.022921, 0.043037, 0.934042));
		// col = adobe_rgb_to_rec2020 * col;
		// The HDR scene is in Rec.709, but the display is Rec.2020
		// col = spectrum::rec709_to_rec2020(col);
	}
#endif
#endif
