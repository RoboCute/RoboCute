...
static float4 QuaternionInvert(float4 q) {
	q.xyz *= -1.f;
	return q;
}

static float4 QuaternionMultiply(float4 q1, float4 q2) {
...
static DualQuaternion DualQuaternionMultiply(DualQuaternion dq1, DualQuaternion dq2) {
...
static DualQuaternion DualQuaternionShortestPath(DualQuaternion dq1, DualQuaternion dq2) {
...
static float4 QuaternionApplyRotation(float4 v, float4 rotQ) {
	v = QuaternionMultiply(rotQ, v);
	return QuaternionMultiply(v, QuaternionInvert(rotQ));
}

static float signNoZero(float x) {
	float s = sign(x);
	if (s)
		return s;
	return 1;
...
static DualQuaternion DualQuaternionFromMatrix4x4(float4x4 m) {
	DualQuaternion dq;

	// http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToQuaternion/index.htm
	// Alternative Method by Christian
	dq.rotation_quaternion.w = sqrt(max(0.0f, 1.0f + m[0][0] + m[1][1] + m[2][2])) / 2.0f;
	dq.rotation_quaternion.x = sqrt(max(0.0f, 1.0f + m[0][0] - m[1][1] - m[2][2])) / 2.0f;
	dq.rotation_quaternion.y = sqrt(max(0.0f, 1.0f - m[0][0] + m[1][1] - m[2][2])) / 2.0f;
	dq.rotation_quaternion.z = sqrt(max(0.0f, 1.0f - m[0][0] - m[1][1] + m[2][2])) / 2.0f;
	...
	dq.translation_quaternion = QuaternionMultiply(dq.translation_quaternion, dq.rotation_quaternion) * 0.5f;
