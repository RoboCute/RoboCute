...
# Type mapping dictionaries (similar to old codegen)
_TYPE_NAMES = {
    bool: "bool",
    int: "int32_t",
    float: "float",
...
def _get_cpp_type(
    ...
    if type_hint in _TYPE_NAMES:
        return _TYPE_NAMES[type_hint]
    ...
    for key, info in all_classes:
        ...
        for method in info.methods:
            ...
            args_expr = _print_arg_vars_decl(
                method.parameters, True, False, True, registry
            )
...
def py_interface_gen(module_name: str, module_filter: List[str] = None) -> str:
    ...
    def get_class_expr(key: str, info: ClassInfo):
        ...
        struct_name = key  # Use full key as struct name for C++ binding
        ...
        def get_method_expr(method: MethodInfo):
            args_decl = _print_py_args_decl(method.parameters, False)
            args_call = _print_py_args(method.parameters, False)
...
def cpp_client_interface_gen(module_filter: List[str] = None, *extra_includes) -> str:
...
def cpp_client_impl_gen(module_filter: List[str] = None, *extra_includes) -> str:
...
def pybind_codegen(
    module_name: str, module_filter: List[str] = None, *extra_includes
) -> str:
    ...
    def get_struct_bindings(key: str, info: ClassInfo):
        ...
        # dispose function
        dispose_name = f"dispose__{key}__"
        ...
        for method in info.methods:
            ...
            return_close = ""
            if method.return_type:
                ret_type = f" -> {_get_full_cpp_type(method.return_type, registry, True, False)}"
                return_expr = "return "

            args_decl = _print_arg_vars_decl(
                method.parameters, False, True, True, registry
            )
            args_call = _print_py_args(method.parameters, True)

            method_func = PYBIND_METHOD_FUNC_TEMPLATE.substitute(
                INDENT=INDENT,
                METHOD_NAME=f"{key}__{method.name}__",
                PTR_NAME=ptr_name,
                ARGS_DECL=args_decl,
                RET_TYPE=ret_type,
                RETURN_EXPR=return_expr,
