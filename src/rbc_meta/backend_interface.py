...
def codegen_pybind(
    ...
    Context.method("create_window", name=tr.string, size=tr.uint2, resizable=tr.bool)
    # mesh
    Context.method(
        "create_mesh",
        data=tr.DataBuffer,
    ...
    # light
    Context.method(
        "add_area_light", matrix=tr.float4x4, luminance=tr.float3, visible=tr.bool
    ).ret_type(tr.VoidPtr)
    Context.method(
        "add_disk_light",
        center=tr.float3,
    ...
    Context.method(
        ...
        luminance=tr.float3,
        forward_dir=tr.float3,
        angle_radians=tr.float,
        small_angle_radians=tr.float,
...
>>>>>>> 78c6c3d (arch adapter)
    ...
    Context.method("update_object_pos", object_ptr=tr.VoidPtr, matrix=tr.float4x4)
    Context.method(
        "update_object",
    ...
    # view
    Context.method("reset_view", resolution=tr.uint2)
    Context.method("reset_frame_index")
    Context.method(
        "set_view_camera", pos=tr.float3, roll=tr.float, pitch=tr.float, yaw=tr.float
    )
    Context.method("disable_view")
    # tick
    Context.method("tick")
