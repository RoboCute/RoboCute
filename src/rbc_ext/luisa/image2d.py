...
class Image2D:
    def __init__(self, width, height, channel, dtype, mip=1, storage=None, external_memory=None):
        ...
        self.dtype = dtype
        self.mip = mip
        self.vectype = dtype if channel == 1 else getattr(
            lcapi, dtype.__name__ + str(channel))
        # default storage type: max precision
        self.storage = storage if storage is not None else getattr(
            lcapi.PixelStorage, dtype.__name__.upper() + str(channel))
        self.format = getattr(
            lcapi, "pixel_storage_to_format_" + dtype.__name__)(self.storage)

        self.bytesize = lcapi.pixel_storage_size(
            self.storage, width, height, 1)
    ...
    def from_array(arr):  # arr: numpy array
        ...
        assert len(
            arr.shape) == 3 and arr.shape[0] > 0 and arr.shape[1] > 0 and arr.shape[2] in (1, 2, 4)
        tex = Image2D.empty(
    ...
    def from_ldr_image(path: str):
        ...
        # save as SRGB 8-bit texture
        tex = Image2D.empty(
            arr.shape[0], arr.shape[1], 4, float, storage="byte")
        tex.copy_from_array(arr)
        return tex
