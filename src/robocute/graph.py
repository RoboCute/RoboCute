...
class NodeGraph:
    ...
    def add_node(
        self, node_id: str, node_type: str, inputs: Optional[Dict[str, Any]] = None
    ) -> Optional[RBCNode]:
        """
        添加节点到图中
        ...
        print(
            f"[NodeGraph]   Scene context available: {self.scene_context is not None}"
        )
    ...
    def get_node_connections(
        ...
        """
        input_connections = [
            conn for conn in self._connections if conn.to_node == node_id
        ]
        output_connections = [
            conn for conn in self._connections if conn.from_node == node_id
        ]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for node_id, node in self._nodes.items():
            ...
            nodes.append({"node_id": node_id, "node_type": node_type, "inputs": inputs})
    ...
    def from_definition(
        ...
        for node_def in definition.nodes:
            print(
                f"[NodeGraph]   Adding node '{node_def.node_id}' (type: {node_def.node_type})"
            )
            ...
            if node is None:
                ...
                raise ValueError(
                    f"Failed to create node: {node_def.node_id} (type: {node_def.node_type})"
                )
        ...
        for conn in definition.connections:
            print(
                f"[NodeGraph]   {conn.from_node}.{conn.from_output} → {conn.to_node}.{conn.to_input}"
            )
            success = graph.add_connection(
                conn.from_node, conn.from_output, conn.to_node, conn.to_input
            )
            if not success:
