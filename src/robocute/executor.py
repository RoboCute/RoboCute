    ...
    def _execute_node(self, node_id: str) -> NodeExecutionResult:
        ...
        if node is None:
            return NodeExecutionResult(
                node_id=node_id, status=ExecutionStatus.FAILED, error="Node not found"
            )

        result = NodeExecutionResult(
            node_id=node_id, status=ExecutionStatus.RUNNING, start_time=datetime.now()
        )
        ...
        try:
            ...
            print(
                f"[Executor] Executing node '{node_id}' (type: {node.__class__.__name__})"
            )
        ...
        try:
            ...
            if execution_order is None:
                ...
                self._execution_result.error = (
                    "Failed to get execution order (cycle detected)"
                )
                self._execution_result.end_time = datetime.now()
                return self._execution_result
            ...
            for node_id in execution_order:
                ...
                if node_result.status == ExecutionStatus.FAILED:
                    print(
                        f"[Executor] ✗ Stopping execution due to node '{node_id}' failure"
                    )
                    self._execution_result.status = ExecutionStatus.FAILED
                    self._execution_result.error = (
            ...
            if self._execution_result.start_time:
                duration = (
                    self._execution_result.end_time - self._execution_result.start_time
                ).total_seconds() * 1000
