...
from robocute.rbc_ext.resource import (
    ResourceManager,
    ResourceHandle,
    ResourceMetadata,
    ResourceType,
...
def create_default_material(
    ...
    material_id = resource_manager.load_material(
        str(temp_path), priority=LoadPriority.High
    )
...
def load_scene_resources(
    ...
    for resource_ref in scene_data.get("resources", []):
        rid = resource_manager.register_resource(
            resource_ref["path"], ResourceType(resource_ref["type"])
        )
...
def preload_with_lod(
    resource_manager: ResourceManager, base_path: str, num_levels: int = 4
) -> List[int]:
    """
...
def manage_memory_budget(
    resource_manager: ResourceManager, entity_count: int, base_budget_mb: int = 1024
):
    """
    ...
    print(
        f"[manage_memory_budget] Set budget to {budget_mb}MB for {entity_count} entities"
    )
...
class ResourceBatch:
    ...
    def add(
        ...
        self.resources.append(
            {"path": path, "type": resource_type, "priority": priority}
        )
    ...
    def load_all(
        ...
        for res in self.resources:
            rid = self.resource_manager.register_resource(res["path"], res["type"])
            self.resource_manager.load_resource(
                rid, priority=res["priority"], on_complete=completion_callback
            )
            resource_ids.append(rid)
...
def wait_for_resources(
    resource_manager: ResourceManager, resource_ids: List[int], timeout: float = 30.0
) -> bool:
    """
